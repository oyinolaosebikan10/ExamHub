...
const registerStudent = async (req, res) => {
    try {
        ...
        // 3. Check that the course is active
        if (!course.isActive) {
            return res.status(400).json({
                success: false,
        ...
        // 5. Normalize the student's name
        const normalizedName = fullName
            .trim()
            .replace(/\s+/g, " ")
            .toLowerCase();

        // 6. Check for an existing student
        ...
        const studentRef = await studentsCollection.add(studentData);
